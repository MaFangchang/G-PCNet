...
def bounding_box_point_cloud(pts: torch.Tensor):
    x = pts[:, 0]
    y = pts[:, 1]
    z = pts[:, 2]
...
def bounding_box_uv_grid(inp: torch.Tensor):
    pts = inp[..., :3].reshape((-1, 3))
    mask = inp[..., 6].reshape(-1)
    point_indices_inside_faces = mask == 1
    pts = pts[point_indices_inside_faces, :]
    return bounding_box_point_cloud(pts)


def center_and_scale_uv_grid(inp: torch.Tensor, return_center_scale=False):
    inp = inp.transpose(1, 3)  # channel last
    bbox = bounding_box_uv_grid(inp)
    diag = bbox[1] - bbox[0]
...
def center_and_scale(data: torch.Tensor):
    data["graph"].ndata["grid"], center, scale = center_and_scale_uv_grid(
...
def rotate_uv_grid(inp, rotation):
    """
...
def load_json_or_pkl(pathname):
    # try to load dataset from pkl first
